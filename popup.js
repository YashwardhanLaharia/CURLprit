...
const filterCategories = {
  ...
  pdf: ['pdf'],
  doc: ['document', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'txt', 'csv', 'xml', 'json'],
  css: ['stylesheet', 'css'],
...
function setupEventListeners() {
  ...
  elements.selectAllCheckbox.addEventListener('change', toggleSelectAllCheckbox);
...
function renderRequests() {
  ...
  elements.selectAllCheckbox.checked = count > 0 && selectedInFiltered === count;
  elements.selectAllCheckbox.indeterminate = selectedInFiltered > 0 && selectedInFiltered < count;
...
function generateCurlForRequest(req, cookiesFromApi = '') {
  ...
  if (req.postData) {
    const postData = typeof req.postData === 'string' ? req.postData : JSON.stringify(req.postData);
    cmd += ` -d '${postData}'`;
  }
